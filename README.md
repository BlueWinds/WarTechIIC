...
* `WIIC_add_{tag}_to_{system} | WIIC_remove_{tag}_from_{system}` (eg: WIIC_add_planet_other_pirate_to_systemded_Sol or WIIC_remove_planet_other_pirate_from_systemdef_Sol) - Setting this will add or remove the given tag from the given system. The tag won't actually added to the company - WIIC 'eats' it.
